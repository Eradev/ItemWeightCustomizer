...
namespace ItemWeightCustomizer
{
    ...
    {
        private static readonly ModKey Campfire = ModKey.FromNameAndExtension("Campfire.esm");
        ...
        public static async Task<int> Main(string[] args)
        {
            return await SynthesisPipeline.Instance.AddPatch<ISkyrimMod, ISkyrimModGetter>(RunPatch)
                .SetAutogeneratedSettings(nickname: "Settings", path: "config.json", out _lazySettings)
        ...
        private static float? FindWeightCategory(string withType, string? editorId)
        {
            ...
            var category = Settings.Categories.FirstOrDefault(x => x.Types.Contains(withType));
        ...
        private static bool? FindItemType(IEnumerable<string> types, string? editorId)
        {
            return types.Any(type => editorId?.Contains(type, StringComparison.OrdinalIgnoreCase) ?? true);
        }
        ...
        private static void RunPatch(IPatcherState<ISkyrimMod, ISkyrimModGetter> state)
        {
            ...
            var ammunitionWeight = weights.Ammunition;
            ...
            if (notesWeight >= 0)
            {
                SynthesisLog($"NOTES, JOURNALS, RECIPES will have their weights multiplied by {notesWeight}");
            }
            ...
            if (scrollWeight >= 0)
            {
                SynthesisLog($"SCROLLS will have their weights multiplied by {scrollWeight}");
            }
            ...
            if (armorWeight >= 0)
            {
                SynthesisLog($"ARMOURS will have their weights multiplied by {armorWeight}");
            }

            if (lightCuirassesWeight >= 0)
            {
                SynthesisLog($"LIGHT ARMOUR CUIRASSES will have their weights multiplied by {lightCuirassesWeight}");
            }

            if (lightOthersWeight >= 0)
            {
                SynthesisLog($"LIGHT ARMOUR will have their weights multiplied by {lightOthersWeight}");
            }

            if (clothesBodyWeight >= 0)
            {
                SynthesisLog($"BODY CLOTHING will have their weights multiplied by {clothesBodyWeight}");
            }

            if (clothesFeetWeight >= 0)
            {
                SynthesisLog($"FEET CLOTHING will have their weights multiplied by {clothesFeetWeight}");
            }

            if (clothingCircletsWeight >= 0)
            {
                SynthesisLog($"CIRCLETS will have their weights multiplied by {clothingCircletsWeight}");
            }

            if (jewelsWeight >= 0)
            {
                SynthesisLog($"JEWELRY will have their weights multiplied by {jewelsWeight}");
            }

            if (weaponWeight >= 0)
            {
                SynthesisLog($"WEAPONS will have their weights multiplied by {weaponWeight}");
            }

            if (foodWeight >= 0)
            {
                SynthesisLog($"FOODS will have their weights multiplied by {foodWeight}");
            }

            if (winesBrandysWeight >= 0)
            {
                SynthesisLog($"WINES, BRANDYS will have their weights multiplied by {winesBrandysWeight}");
            }

            if (alesMeadsWeight >= 0)
            {
                SynthesisLog($"ALES, MEADS will have their weights multiplied by {alesMeadsWeight}");
            }

            if (breadsFloursWeight >= 0)
            {
                SynthesisLog($"BREADS, FLOURS will have their weights multiplied by {breadsFloursWeight}");
            }

            if (vegetablesWeight >= 0)
            {
                SynthesisLog($"VEGETABLE SOUPS will have their weights multiplied by {vegetablesWeight}");
            }

            if (soupsWeight >= 0)
            {
                SynthesisLog($"SOUPS will have their weights multiplied by {soupsWeight}");
            }

            if (meatsWeight >= 0)
            {
                SynthesisLog($"MEATS will have their weights multiplied by {meatsWeight}");
            }

            if (seafoodsWeight >= 0)
            {
                SynthesisLog($"SEAFOODS will have their weights multiplied by {seafoodsWeight}");
            }

            if (drinksWeight >= 0)
            {
                SynthesisLog($"DRINKS will have their weights multiplied by {drinksWeight}");
            }

            if (potionWeight >= 0)
            {
                SynthesisLog($"POTIONS will have their weights multiplied by {potionWeight}");
            }

            if (ingotWeight >= 0)
            {
                SynthesisLog($"INGOTS will have their weights multiplied by {ingotWeight}");
            }

            if (gemWeight >= 0)
            {
                SynthesisLog($"GEMS will have their weights multiplied by {gemWeight}");
            }

            if (toolsWeight >= 0)
            {
                SynthesisLog($"TOOLS will have their weights multiplied by {toolsWeight}");
            }

            if (tentsWeight >= 0)
            {
                SynthesisLog($"TENTS will have their weights multiplied by {tentsWeight}");
            }

            if (silverwareWeight >= 0)
            {
                SynthesisLog($"SILVERWARE will have their weights multiplied by {silverwareWeight}");
            }

            if (emptyBottlesWeight >= 0)
            {
                SynthesisLog($"EMPTY BOTTLES will have their weights multiplied by {emptyBottlesWeight}");
            }

            if (animalPeltsWeight >= 0)
            {
                SynthesisLog($"ANIMAL PELTS will have their weights multiplied by {animalPeltsWeight}");
            }

            if (animalPartWeight >= 0)
            {
                SynthesisLog($"ANIMAL PARTS will have their weights multiplied by {animalPartWeight}");
            }

            if (animalHideWeight >= 0)
            {
                SynthesisLog($"ANIMAL HIDES will have their weights multiplied by {animalHideWeight}");
            }

            if (clutterWeight >= 0)
            {
                SynthesisLog($"CLUTTER will have their weights multiplied by {clutterWeight}");
            }

            if (miscWeight >= 0)
            {
                SynthesisLog($"MISCELLANEOUS ITEMS will have their weights multiplied by {miscWeight}");
            }

            if (ammunitionWeight >= 0)
            {
                SynthesisLog($"AMMUNITION will have their weights multiplied by {ammunitionWeight}");
            }
                ...
                notesWeight >= 0 || ActionableCategoryExists("notes"))
            {
                ...
                foreach (var book in
                         state.LoadOrder.PriorityOrder.WinningOverrides<IBookGetter>())
                {
            ...
            if (ingredientWeight >= 0 || ActionableCategoryExists("ingredients"))
            {
                foreach (var ingredient in
                         state.LoadOrder.PriorityOrder.WinningOverrides<IIngredientGetter>())
                {
            ...
            if (scrollWeight >= 0 || ActionableCategoryExists("scrolls"))
            {
                foreach (var scroll in
                         state.LoadOrder.PriorityOrder.WinningOverrides<IScrollGetter>())
                {
            ...
            if (soulGemWeight >= 0 || ActionableCategoryExists("soulgems"))
            {
                foreach (var soulGem in
                         state.LoadOrder.PriorityOrder.WinningOverrides<ISoulGemGetter>())
                {
                ...
                )
            {
                foreach (var armor in
                         state.LoadOrder.PriorityOrder.WinningOverrides<IArmorGetter>())
                {
                    ...
                    {
                        if (armor.Keywords?.Any(link => link.FormKey.Equals(ArmorCuirass.FormKey)) ?? false)
                        {
                            newWeight = FindWeightCategory("lightCuirasses", armor.EditorID) ?? lightCuirassesWeight;
                        }
                        else
                        {
                            newWeight = FindWeightCategory("lightOthers", armor.EditorID) ?? lightOthersWeight;
                        }
                    }

                    if (armor.BodyTemplate?.ArmorType == ArmorType.Clothing)
                    {
                        if (armor.Keywords?.Any(link => link.FormKey.Equals(ArmorJewelry.FormKey)) ?? false)
                        {
                            newWeight = FindWeightCategory("jewels", armor.EditorID) ?? jewelsWeight;
                        }

                        if (armor.Keywords?.Any(link => link.FormKey.Equals(ClothingCirclet.FormKey)) ?? false)
                        {
                            newWeight = FindWeightCategory("clothingCirclets", armor.EditorID) ?? clothingCircletsWeight;
                        }

                        if (armor.Keywords?.Any(link => link.FormKey.Equals(ClothingFeet.FormKey)) ?? false)
                        {
                            newWeight = FindWeightCategory("clothesFeet", armor.EditorID) ?? clothesFeetWeight;
                        }

                        if (armor.Keywords?.Any(link => link.FormKey.Equals(ClothingBody.FormKey)) ?? false)
                        {
            ...
            if (weaponWeight >= 0 || ActionableCategoryExists("weapons"))
            {
                foreach (var weapon in
                         state.LoadOrder.PriorityOrder.WinningOverrides<IWeaponGetter>())
                {
            ...
            // ***** FOODS & POTIONS ***** //
            if (foodWeight >= 0 || ActionableCategoryExists("foods") ||
                potionWeight >= 0 || ActionableCategoryExists("potions") ||
                ...
                )
            {
                ...
                foreach (var ingestible in
                         state.LoadOrder.PriorityOrder.WinningOverrides<IIngestibleGetter>())
                {
                    float newWeight = -1;
                    if (ingestible.Keywords?.Any(link => link.FormKey.Equals(VendorItemFood.FormKey)) ?? false)
                    {
                        ...
                        if (FindItemType(wines, ingestible.EditorID) ?? false)
                        {
                            newWeight = FindWeightCategory("winesAles", ingestible.EditorID) ?? winesBrandysWeight;
                        }
                        else if (FindItemType(breads, ingestible.EditorID) ?? false)
                        {
                            newWeight = FindWeightCategory("breadsFlours", ingestible.EditorID) ?? breadsFloursWeight;
                        }
                        else if ((ingestible.EditorID?.Contains("soup", StringComparison.OrdinalIgnoreCase) ?? false) &&
                                 (ingestible.EditorID?.Contains("veg", StringComparison.OrdinalIgnoreCase) ?? false))
                        {
                            newWeight = FindWeightCategory("vegetables", ingestible.EditorID) ?? vegetablesWeight;
                        }
                        else if (FindItemType(soups, ingestible.EditorID) ?? false)
                        {
                            newWeight = FindWeightCategory("soups", ingestible.EditorID) ?? soupsWeight;
                        }
                        else if (ingestible.EditorID?.Contains("meat", StringComparison.OrdinalIgnoreCase) ?? false)
                        {
                            newWeight = FindWeightCategory("meats", ingestible.EditorID) ?? meatsWeight;
                        }
                        else if (ingestible.EditorID?.Contains("sea", StringComparison.OrdinalIgnoreCase) ?? false)
                        {
                            newWeight = FindWeightCategory("seafoods", ingestible.EditorID) ?? seafoodsWeight;
                        }
                        else if (ingestible.EditorID?.Contains("drink", StringComparison.OrdinalIgnoreCase) ?? false)
                        {
                            newWeight = FindWeightCategory("drinks", ingestible.EditorID) ?? drinksWeight;
                        }
                    }
                    else
                    {
            ...
            {
                foreach (var item in
                         state.LoadOrder.PriorityOrder.WinningOverrides<IMiscItemGetter>())
                {
            ...
            if (ammunitionWeight >= 0 || ActionableCategoryExists("ammunition"))
            {
                foreach (var ammunition in
                         state.LoadOrder.PriorityOrder.WinningOverrides<IAmmunitionGetter>())
                {
